...
genotypeFactor.card=length(genotypesToAlleles);
genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
for i=1:length(allelesToGenotypes)
    for j=1:length(allelesToGenotypes)
    index=allelesToGenotypes(i,j);
    genotypeFactor.val(index)=genotypeFactor.val(index)+alleleFreqs(i)*alleleFreqs(j);
    end
end
